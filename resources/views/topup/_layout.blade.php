{{--
    Generic top-up page.
    Required vars: $user, $packages, $gameName
    Required vars (page-specific): $gameImage, $gameDesc, $gameIdLabel, $gameIdPlaceholder, $gameIdHint, $processRoute, $needsZoneId (bool, optional)
--}}
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-saldo { font-size: 13px; color: rgba(255,255,255,0.45); }
        ...
        .hero-img {
            ...
            overflow: hidden; flex-shrink: 0; background: rgba(255,255,255,0.08);
        }
        ...
        .id-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 28px; }
        ...
        .field input:focus { border-color: rgba(99,57,255,0.7); background: rgba(99,57,255,0.07); }
        .field-hint { font-size: 11px; color: rgba(255,255,255,0.3); margin-top: 6px; }
        .field-error-msg { font-size: 11.5px; color: #f87171; margin-top: 5px; }

        .pkg-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 10px; margin-bottom: 28px; }
        ...
        .pkg-card {
            position: relative; border: 1.5px solid rgba(255,255,255,0.08);
        ...
        .pkg-card:hover { border-color: rgba(99,57,255,0.5); background: rgba(99,57,255,0.08); transform: translateY(-2px); }
        .pkg-card.selected { border-color: #6339ff; background: rgba(99,57,255,0.15); }
        .pkg-card.selected::after { content: '✓'; position: absolute; top: 6px; right: 9px; font-size: 11px; color: #a78bff; font-weight: 700; }
        .pkg-card input[type="radio"] { position: absolute; opacity: 0; pointer-events: none; }
        .pkg-label { font-size: 12.5px; font-weight: 700; color: #ffffff; margin-bottom: 5px; }
        .pkg-price { font-size: 11.5px; color: rgba(255,255,255,0.4); }
    ...
    <form method="POST" action="{{ route($processRoute) }}">
        ...
        <div class="id-row">
            <div class="field">
                ...
                <input type="text" id="user_id_game" name="user_id_game"
            ...
            @foreach($packages as $pkg)
                <label class="pkg-card {{ old('package_id') == $pkg['id'] ? 'selected' : '' }}">
                    <input type="radio" name="package_id" value="{{ $pkg['id'] }}"
                        {{ old('package_id') == $pkg['id'] ? 'checked' : '' }} required>
